...
async def support_on(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "💬 <b>Режим підтримки активовано.</b>\n\n"
        "Напишіть ваше повідомлення — ми відповімо найближчим часом.\n\n"
        "Щоб завершити — натисніть <b>«❌ Завершити підтримку»</b>.",
        reply_markup=support_active_keyboard(),
        parse_mode="HTML",
    )


@router.message(F.text == "❌ Завершити підтримку")
async def support_off(message: Message, state: FSMContext) -> None:
    ...
    await state.clear()
    await message.answer(
        "✅ Підтримку завершено. Повертаємось до головного меню.",
...
async def forward_user_to_admin(message: Message) -> None:
    ...
    await message.answer("📨 Надіслано. Зачекайте відповіді від адміністратора.")
